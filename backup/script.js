...
class WaveMusicPlayer {
    ...
    setupEventListeners() {
        ...
        this.volumeSlider.addEventListener('mousedown', (e) => {
            ...
            document.addEventListener('mousemove', this.onVolumeDrag.bind(this));
            document.addEventListener('mouseup', this.onVolumeDragEnd.bind(this));
        });
    ...
    initTheme() {
        const savedTheme = localStorage.getItem('wavePlayerTheme') || 'dark';
        document.documentElement.setAttribute('data-theme', savedTheme);
        this.themeToggleSwitch.checked = savedTheme === 'dark';
    }

    toggleTheme() {
        const newTheme = this.themeToggleSwitch.checked ? 'dark' : 'light';
        document.documentElement.setAttribute('data-theme', newTheme);
        localStorage.setItem('wavePlayerTheme', newTheme);
    }
    
    async handleClearLibrary() {
        if (confirm("Are you sure you want to delete all tracks from your library? This action cannot be undone.")) {
    ...
    async loadPersistedTracks() {
        const dbTracks = await loadTracksFromDB(this.db);
        if (dbTracks && dbTracks.length > 0) {
            this.tracks = dbTracks.map(track => {
                // Re-create object URL from the stored file blob
                if (track.originalFile instanceof Blob) {
                    track.file = URL.createObjectURL(track.originalFile);
                }
                return track;
            });
            this.showAlert(`${this.tracks.length} tracks loaded from your library.`, 'success');
        } else {
        ...
        for (const file of files) {
            if (file.type.startsWith('audio/')) {
                ...
                const track = {
                    ...
                    originalFile: file, // Keep the original file object for DB
                    duration: null,
                    id: trackId
                };

                this.tracks.push(track);
                await saveTrackToDB(this.db, track); // Save to IndexedDB
                this.extractMetadata(file, track); // This will re-render and re-save
                addedCount++;
            }
        }
        if (addedCount > 0) {
            this.showAlert(`${addedCount} track(s) added to your library.`, 'success');
            this.renderContentView(this.currentView);
        }
    }
        ...
        if (window.jsmediatags) {
            jsmediatags.read(file, {
                onSuccess: async (tag) => {
                    ...
                    if (tags.picture) {
                        const base64String = this.arrayBufferToBase64(tags.picture.data);
                        track.artwork = `data:${tags.picture.format};base64,${base64String}`;
                    }
                    this.renderContentView(this.currentView);
                    // Re-save track to DB with updated metadata
    ...
    // --- Player Logic (with Title Update) ---
    handlePlay() {
        this.isPlaying = true;
    ...
    // --- The rest of the JS is largely unchanged ---
    // (Pasting the rest for completeness, no major logical changes needed for these parts)
    ...
    playTrack(index) { if (index >= 0 && index < this.tracks.length) { this.currentIndex = index; const track = this.tracks[index]; this.audio.src = track.file; this.audio.load(); this.currentTitle.textContent = track.title; this.currentArtist.textContent = track.artist; if (track.artwork) { this.albumArtwork.innerHTML = `<img src="${track.artwork}" alt="Album Art">`; } else { this.albumArtwork.innerHTML = `<div class="album-placeholder"><i class="ri-disc-line"></i></div>`; } this.audio.play(); this.renderContentView(this.currentView); this.addRecentPlay(index); } }
    ...
    toggleShuffle() { this.isShuffle = !this.isShuffle; this.shuffleBtn.classList.toggle('active', this.isShuffle); }
    toggleRepeat() { this.repeatMode = (this.repeatMode + 1) % 3; const icons = ['ri-repeat-line', 'ri-repeat-line', 'ri-repeat-one-line']; this.repeatBtn.innerHTML = `<i class="${icons[this.repeatMode]}"></i>`; this.repeatBtn.classList.toggle('active', this.repeatMode > 0); }
    toggleMute() { this.audio.muted = !this.audio.muted; const icon = this.audio.muted ? 'ri-volume-mute-line' : 'ri-volume-up-line'; this.volumeBtn.innerHTML = `<i class="${icon}"></i>`; this.volumeFill.style.width = this.audio.muted ? '0%' : (this.volume * 100) + '%'; }
    seekTo(event) { if (isNaN(this.audio.duration)) return; const rect = this.progressBar.getBoundingClientRect(); const percent = (event.clientX - rect.left) / rect.width; const seekTime = percent * this.audio.duration; this.audio.currentTime = seekTime; }
    setVolume(event) { const rect = this.volumeSlider.getBoundingClientRect(); const percent = Math.max(0, Math.min(1, (event.clientX - rect.left) / rect.width)); this.volume = percent; this.audio.volume = this.volume; this.volumeFill.style.width = (this.volume * 100) + '%'; const icon = this.volume === 0 ? 'ri-volume-mute-line' : 'ri-volume-up-line'; this.volumeBtn.innerHTML = `<i class="${icon}"></i>`; this.audio.muted = false; }

    // --- Volume Drag and Scroll Handlers ---
    onVolumeDrag(e) {
    ...
    onVolumeDragEnd() {
        ...
        document.removeEventListener('mousemove', this.onVolumeDrag.bind(this));
        document.removeEventListener('mouseup', this.onVolumeDragEnd.bind(this));
    }

    adjustVolumeOnScroll(e) {
        e.preventDefault();
        ...
        const icon = this.volume === 0 ? 'ri-volume-mute-line' : 'ri-volume-up-line';
        this.volumeBtn.innerHTML = `<i class="${icon}"></i>`;
        this.audio.muted = false;
    }
    updateProgress() { if (this.audio.duration) { const percent = (this.audio.currentTime / this.audio.duration) * 100; this.progressFill.style.width = percent + '%'; this.currentTime.textContent = this.formatTime(this.audio.currentTime); } }
    updateDuration() { this.totalTime.textContent = this.formatTime(this.audio.duration); if (this.tracks[this.currentIndex]) { this.tracks[this.currentIndex].duration = this.formatTime(this.audio.duration); } this.renderContentView(this.currentView); }
    ...
    toggleFavorite(trackIndex, event) { event.stopPropagation(); const trackTitle = this.tracks[trackIndex].title; if (this.favorites.has(trackIndex)) { this.favorites.delete(trackIndex); this.showAlert(`Removed "${trackTitle}" from Favorites.`, 'info'); } else { this.favorites.add(trackIndex); this.showAlert(`Added "${trackTitle}" to Favorites.`, 'success'); } const favoriteBtn = event.currentTarget; const icon = favoriteBtn.querySelector('i'); favoriteBtn.classList.toggle('active'); icon.className = this.favorites.has(trackIndex) ? 'ri-heart-fill' : 'ri-heart-line'; if(this.currentView === 'favorites') { this.renderContentView('favorites'); } }
    ...
    handleKeyboard(event) { if (event.target.tagName === 'INPUT') return; switch(event.code) { case 'Space': event.preventDefault(); this.togglePlay(); break; case 'ArrowLeft': if (isNaN(this.audio.duration)) return; event.preventDefault(); this.audio.currentTime = Math.max(0, this.audio.currentTime - (event.shiftKey ? 30 : 10)); break; case 'ArrowRight': if (isNaN(this.audio.duration)) return; event.preventDefault(); this.audio.currentTime = Math.min(this.audio.duration, this.audio.currentTime + (event.shiftKey ? 30 : 10)); break; case 'KeyN': if (event.ctrlKey || event.metaKey) return; event.preventDefault(); this.nextTrack(); break; case 'KeyP': if (event.ctrlKey || event.metaKey) return; event.preventDefault(); this.previousTrack(); break; case 'ArrowUp': event.preventDefault(); this.volume = Math.min(1, this.audio.volume + 0.1); this.audio.volume = this.volume; this.volumeFill.style.width = (this.volume * 100) + '%'; break; case 'ArrowDown': event.preventDefault(); this.volume = Math.max(0, this.audio.volume - 0.1); this.audio.volume = this.volume; this.volumeFill.style.width = (this.volume * 100) + '%'; break; case 'KeyS': if (event.ctrlKey || event.metaKey) event.preventDefault(); else this.toggleShuffle(); break; case 'KeyR': if (event.ctrlKey || event.metaKey) event.preventDefault(); else this.toggleRepeat(); break; case 'KeyM': if (event.ctrlKey || event.metaKey) event.preventDefault(); else this.toggleMute(); break; case 'Slash': if (event.ctrlKey || event.metaKey) { event.preventDefault(); this.searchInput.focus(); } break; } }
}
