...
class WaveMusicPlayer {
    ...
    async initApp() {
        ...
        if (this.settings.rememberPosition) { // rememberPositionがtrueの場合のみ
            const savedPosition = JSON.parse(localStorage.getItem('wavePlayerLastPosition'));
            if (savedPosition && typeof savedPosition.index === 'number' && typeof savedPosition.time === 'number') {
                // 保存された曲と時間があれば、その曲をその時間から再生する
                this.playTrack(savedPosition.index, savedPosition.time);
            }
        }
    }
    ...
    initElements() {
        ...
        // Track list and content view container
        ...
        this.rememberPositionToggle = document.getElementById('rememberPositionToggle');
        ...
        this.fullscreenPlaylistModal = document.getElementById('fullscreenPlaylistModal');
        this.closeFullscreenPlaylistBtn = document.getElementById('closeFullscreenPlaylistBtn');
        this.fullscreenPlaylistList = document.getElementById('fullscreenPlaylistList');
        this.newPlaylistFullscreenBtn = document.getElementById('newPlaylistFullscreenBtn');
    }
    ...
    setupEventListeners() {
        ...
        if (this.progressSection) {
            this.progressSection.addEventListener('click', (e) => this.seekTo(e, this.progressBar));
        } else {
            this.progressBar.addEventListener('click', (e) => this.seekTo(e, this.progressBar));
        }
        this.volumeSlider.addEventListener('click', (e) => {
            e.stopPropagation();
            this.setVolumeFromSlider(e, this.volumeSlider);
        });
        ...
        this.clearLibraryBtn.addEventListener('click', () => this.promptClearLibrary());

        // New settings listeners
        this.crossfadeToggle.addEventListener('change', (e) => this.setSetting('crossfade', e.target.checked));
        this.rememberPositionToggle.addEventListener('change', (e) => this.setSetting('rememberPosition', e.target.checked));

        // Confirmation Modal Listeners
        this.confirmClearBtn.addEventListener('click', () => this.handleClearLibrary());
        this.cancelClearBtn.addEventListener('click', () => this.closeConfirmationModal());
        this.confirmationModal.querySelector('.modal-overlay').addEventListener('click', () => this.closeConfirmationModal());

        // Fullscreen Player Events
        this.controlsFooter.addEventListener('click', (e) => {
            if (e.target.closest('button, .progress-section, .volume-slider, .track-actions, a')) {
                return;
            }
            this.openPlayer();
        });
        ...
        this.fullscreenPlaylistBtn.addEventListener('click', () => this.openFullscreenPlaylistModal());
        ...
        // Fullscreen Playlist Modal Events
        this.closeFullscreenPlaylistBtn.addEventListener('click', () => this.closeFullscreenPlaylistModal());
        this.fullscreenPlaylistModal.querySelector('.modal-overlay').addEventListener('click', () => this.closeFullscreenPlaylistModal());
        this.newPlaylistFullscreenBtn.addEventListener('click', () => {
            this.closeFullscreenPlaylistModal();
            this.promptCreatePlaylist(this.currentIndex, true);
        });
    }
    ...
    initTheme() { const savedTheme = localStorage.getItem('wavePlayerTheme') || 'purple'; document.documentElement.setAttribute('data-theme', savedTheme); this.themeToggleSwitch.checked = savedTheme === 'blue'; }
    toggleTheme() { const newTheme = this.themeToggleSwitch.checked ? 'blue' : 'purple'; document.documentElement.setAttribute('data-theme', newTheme); localStorage.setItem('wavePlayerTheme', newTheme); }
    ...
    async handleClearLibrary() {
        ...
        this.showAlert('Library cleared. The application will now reload.', 'info', 4000);
        setTimeout(() => window.location.reload(), 4000);
    }

    loadSettings() {
        const savedSettings = JSON.parse(localStorage.getItem('wavePlayerSettings'));
        if (savedSettings) {
            this.settings = savedSettings;
        }
        this.crossfadeToggle.checked = this.settings.crossfade;
        this.rememberPositionToggle.checked = this.settings.rememberPosition;
    }

    setSetting(key, value) {
        this.settings[key] = value;
        localStorage.setItem('wavePlayerSettings', JSON.stringify(this.settings));
        this.showAlert(`${key.replace(/([A-Z])/g, ' $1').toLowerCase()} is now ${value ? 'enabled' : 'disabled'}.`, 'info');
    }
    ...
    playTrack(index, seekTime = 0) {
        if (index >= 0 && index < this.tracks.length) {
            const prevIndex = this.currentIndex; // 以前のcurrentIndexを保存
            ...
            // メタデータ読み込み時の処理
            const onLoadedMetadataForSeek = () => {
                if (seekTime > 0 && this.audio.readyState >= 2) {
                    this.audio.currentTime = seekTime;
                    this.audio.removeEventListener('loadedmetadata', onLoadedMetadataForSeek);
                }
            };
            this.audio.addEventListener('loadedmetadata', onLoadedMetadataForSeek);
            ...
            if (playPromise !== undefined) {
                playPromise.then(() => {
                    ...
                    // アクティブなトラックのUIを更新
                    this._updateTrackActiveState(prevIndex, this.currentIndex);
                    this.addRecentPlay(index);
                    this.updateFavoriteButtons();

                    // スクロール処理を確実に実行
                    requestAnimationFrame(() => {
                        this._scrollCurrentTrackIntoView();
                    });
    ...
    // 新しく追加したアクティブ状態更新メソッド
    _updateTrackActiveState(oldIndex, newIndex) {
        // 古いトラックのアクティブ状態を解除
        const oldTrackElement = document.querySelector(`.track-item[data-index="${oldIndex}"]`);
        if (oldTrackElement) {
            oldTrackElement.classList.remove('active');
        }

        // 新しいトラックをアクティブに
        const newTrackElement = document.querySelector(`.track-item[data-index="${newIndex}"]`);
        if (newTrackElement) {
    ...
    savePlaybackPosition() {
        // rememberPosition がオンで、かつ現在曲が再生されている場合のみ保存する
        if (this.settings.rememberPosition && this.audio.src && this.tracks.length > 0) {
            const position = {
                index: this.currentIndex,
                time: this.audio.currentTime
            };
            localStorage.setItem('wavePlayerLastPosition', JSON.stringify(position));
        } else {
            // 設定がオフの場合や、再生中でない場合は保存された位置をクリアする
            localStorage.removeItem('wavePlayerLastPosition');
        }
    }
    ...
    updateDuration() {
        ...
        // 再生中のトラックのdurationを直接更新
        if (this.tracks[this.currentIndex]) {
            this.tracks[this.currentIndex].duration = durationFormatted; // データモデルを更新

            // DOM上のduration表示も直接更新
            const currentTrackElement = document.querySelector(`.track-item[data-index="${this.currentIndex}"]`);
            if (currentTrackElement) {
                const durationElement = currentTrackElement.querySelector('.track-duration');
                if (durationElement) {
    ...
    _scrollCurrentTrackIntoView() {
        console.log('スクロール処理開始（track-listをスクロール）');

        // スクロールコンテナ（track-list）を取得
        const scrollContainer = document.querySelector('.track-list');
        if (!scrollContainer) {
            console.error('スクロールコンテナ（track-list）が見つかりません');
            return;
        }

        // 現在のトラック要素を取得
        const currentTrackElement = scrollContainer.querySelector(`.track-item[data-index="${this.currentIndex}"]`);
        if (!currentTrackElement) {
            console.error('現在のトラック要素が見つかりません:', this.currentIndex);
            return;
        }

        // スクロール位置を計算して設定（1回のみ実行）
        const containerHeight = scrollContainer.offsetHeight;
        const elementOffset = currentTrackElement.offsetTop;
        const elementHeight = currentTrackElement.offsetHeight;
        ...
        setTimeout(() => {
            console.log('スクロール完了:', {
                ...
                difference: Math.abs(scrollContainer.scrollTop - scrollPosition)
            });
        }, 300);
    }
    ...
    async loadSampleTracks() {
        const sampleTracksData = [
            { title: 'Midnight Dreams', artist: 'Ethereal Sounds', file: './sounds/sample1.mp3' },
            { title: 'Neon Lights', artist: 'Cyber Wave', file: './sounds/sample2.mp3' },
            { title: 'Ocean Breeze', artist: 'Ambient Flow', file: './sounds/sample3.mp3' },
        ];
        for (const trackData of sampleTracksData) {
            try {
                const response = await fetch(trackData.file);
                if (response.ok) {
                    const fileBlob = await response.blob();
                    const file = new File([fileBlob], trackData.file.split('/').pop(), { type: fileBlob.type });
                    const track = { ...trackData, duration: null, id: this.generateUniqueId(), originalFile: file, file: URL.createObjectURL(file) };
                    this.tracks.push(track);
                    await saveTrackToDB(this.db, track);
                }
            } catch (error) {
                console.log(`Sample track ${trackData.file} not found`);
            }
        }
        if (this.tracks.length > 0) this.showAlert('Added sample tracks to get you started.', 'info');
    }
    ...
    renderContentView(view) {
        this.contentViewContainer.innerHTML = '';
        this.searchInput.value = '';
        let contentHtml = '';
        switch (view) {
            case 'library':
                contentHtml = this.renderLibraryView();
                break;
            case 'playlist':
                contentHtml = this.renderPlaylistView();
                break;
            case 'favorites':
                contentHtml = this.renderFavoritesView();
                break;
            case 'recent':
                contentHtml = this.renderRecentView();
                break;
            default:
                contentHtml = this.renderLibraryView();
                break;
        }
        this.contentViewContainer.innerHTML = contentHtml;
        this.attachEventListenersToView(view);
        // ★★★ ここからスクロール処理を削除 (playTrackで直接呼ぶようになったため) ★★★
    ...
    attachEventListenersToView(view) {
        this.attachEventListenersToTrackList();
        if (view === 'library') {
            document.getElementById('audioFiles').addEventListener('change', (e) => this.handleFiles(e));
        } else if (view === 'playlist') {
            const createPlaylistBtn = document.getElementById('createPlaylistBtn');
            if (createPlaylistBtn) {
                createPlaylistBtn.addEventListener('click', () => this.promptCreatePlaylist());
            }
            this.playlists.forEach((playlist) => {
                const playlistItem = document.getElementById(`playlist-${playlist.id}`);
                if (playlistItem) {
                    playlistItem.addEventListener('click', (e) => {
                        if (!e.target.closest('.action-btn')) {
                            this.renderPlaylistTracks(playlist.id);
                        }
                    });
    ...
    renderTrackList(tracksToDisplay, showActions = true) {
        if (tracksToDisplay.length === 0) {
            return `<div class="empty-state"><i class="ri-music-2-line empty-icon"></i><div class="empty-title">No music found</div><div class="empty-subtitle">Add music files to your library.</div></div>`;
        }
        let trackListHtml = `<div class="track-list">`;
        tracksToDisplay.forEach((track) => {
            ...
            const isFavorite = this.favorites.has(originalIndex);
            trackListHtml += `<div class="track-item ${isActive ? 'active' : ''}" data-index="${originalIndex}"><div class="track-number">${String(tracksToDisplay.indexOf(track) + 1).padStart(2, '0')}</div><div class="track-info"><div class="track-title">${track.title}</div><div class="track-artist">${track.artist}</div></div><div class="track-duration">${track.duration || '--:--'}</div>${showActions ? `<div class="track-actions"><button class="action-btn favorite-btn ${isFavorite ? 'active' : ''}" data-index="${originalIndex}" title="${isFavorite ? 'Remove from Favorites' : 'Add to Favorites'}"><i class="${isFavorite ? 'ri-heart-fill' : 'ri-heart-line'}"></i></button><button class="action-btn add-to-playlist-btn" data-index="${originalIndex}" title="Add to Playlist"><i class="ri-add-line"></i></button></div>` : ''}</div>`;
        });
        trackListHtml += `</div>`;
        return trackListHtml;
    }
    attachEventListenersToTrackList() {
        document.querySelectorAll('.track-item').forEach(item => {
            const index = parseInt(item.dataset.index);
            if (isNaN(index)) return;
            item.addEventListener('click', (e) => {
                if (!e.target.closest('.action-btn') && !e.target.closest('.playlist-item')) {
        ...
        document.querySelectorAll('.favorite-btn').forEach(btn => {
            ...
            btn.addEventListener('click', (e) => this.toggleFavorite(index, e));
        });
        document.querySelectorAll('.add-to-playlist-btn').forEach(btn => {
            const index = parseInt(btn.dataset.index);
            btn.addEventListener('click', (e) => this.promptAddToPlaylist(index, e));
        });
    }
    ...
    renderPlaylistTracks(playlistId) {
        ...
        if (!playlist) return;
        const playlistTracks = playlist.trackIds.map(index => this.tracks[index]).filter(Boolean);
        const contentHtml = `<div class="list-header"><button class="back-btn" id="backToPlaylistsBtn"><i class="ri-arrow-left-line"></i> Back to Playlists</button><h1 class="list-title">${playlist.name}</h1></div>${this.renderTrackList(playlistTracks)}`;
        this.contentViewContainer.innerHTML = contentHtml;
        document.getElementById('backToPlaylistsBtn').addEventListener('click', () => this.renderContentView('playlist'));
        this.attachEventListenersToTrackList();
    }
    renderFavoritesView() {
        const favoriteTracks = Array.from(this.favorites).map(index => this.tracks[index]).filter(Boolean);
        return `<div class="list-header"><h1 class="list-title">Your Favorites</h1></div>${favoriteTracks.length === 0 ? `<div class="empty-state"><i class="ri-heart-line empty-icon"></i><div class="empty-title">No favorite tracks yet</div><div class="empty-subtitle">Click the heart icon on any track to add it!</div></div>` : this.renderTrackList(favoriteTracks)}`;
    }
    renderRecentView() {
        const recentTracks = this.recentPlays.map(index => this.tracks[index]).filter(Boolean);
        return `<div class="list-header"><h1 class="list-title">Recently Played</h1></div>${recentTracks.length === 0 ? `<div class="empty-state"><i class="ri-time-line empty-icon"></i><div class="empty-title">No recent plays</div><div class="empty-subtitle">Start listening to see your history!</div></div>` : this.renderTrackList(recentTracks, false)}`;
    }

    promptCreatePlaylist(trackIndexToAddAfterCreation = null, fromFullscreen = false) {
        this.closeModal();
        const modalHTML = `<div class="modal-overlay"></div><div class="modal-content create-playlist-modal"><div class="modal-header">Create New Playlist</div><div class="modal-body"><input type="text" id="newPlaylistName" class="modal-input" placeholder="My Awesome Playlist"></div><div class="modal-footer"><button class="modal-button" id="cancelPlaylistCreation">Cancel</button><button class="modal-button-primary" id="confirmPlaylistCreation">Create</button></div></div>`;
        this.modalContainer.innerHTML = modalHTML;
        const input = document.getElementById('newPlaylistName');
        input.focus();
        const createAction = () => {
            const playlistName = input.value;
            const newPlaylist = this.createPlaylist(playlistName);
            if (newPlaylist && trackIndexToAddAfterCreation !== null) {
                this.addTrackToPlaylist(newPlaylist.id, trackIndexToAddAfterCreation);
            }
            this.closeModal();
            if (fromFullscreen) {
                this.openFullscreenPlaylistModal();
            }
        };
        document.getElementById('confirmPlaylistCreation').addEventListener('click', createAction);
        document.getElementById('cancelPlaylistCreation').addEventListener('click', () => this.closeModal());
        this.modalContainer.querySelector('.modal-overlay').addEventListener('click', () => this.closeModal());
        input.addEventListener('keydown', (e) => { if (e.key === 'Enter') createAction(); });
    }

    createPlaylist(name) {
        if (!name || !name.trim()) { this.showAlert("Playlist name cannot be empty!", 'error'); return null; }
        name = name.trim();
        if (this.playlists.some(p => p.name.toLowerCase() === name.toLowerCase())) { this.showAlert(`Playlist "${name}" already exists!`, 'error'); return null; }
        const newPlaylist = { id: this.generateUniqueId(), name, trackIds: [] };
        this.playlists.push(newPlaylist);
        this.showAlert(`Playlist "${name}" created.`, 'success');
        if (this.currentView === 'playlist') { this.renderContentView('playlist'); }
        this.renderFullscreenPlaylistModal();
        return newPlaylist;
    }
    ...
    deletePlaylist(playlistId) {
        if (confirm("Are you sure you want to delete this playlist? This cannot be undone.")) {
            ...
            if (playlistIndex > -1) {
                const playlistName = this.playlists[playlistIndex].name;
                this.playlists.splice(playlistIndex, 1);
                this.showAlert(`Playlist "${playlistName}" deleted.`, 'info');
                this.renderContentView('playlist');
                this.renderFullscreenPlaylistModal();
            }
        }
    }

    promptAddToPlaylist(trackIndex, event) {
        if (event) event.stopPropagation();
        this.openFullscreenPlaylistModal(trackIndex);
    }

    addTrackToPlaylist(playlistId, trackIndex) {
        const playlist = this.playlists.find(p => p.id === playlistId);
        if (playlist) {
            if (!playlist.trackIds.includes(trackIndex)) {
                playlist.trackIds.push(trackIndex);
                this.showAlert(`Added "${this.tracks[trackIndex].title}" to "${playlist.name}".`, 'success');
                if (this.currentView === 'playlist') { this.renderContentView('playlist'); }
            } else {
                this.showAlert(`Track is already in "${playlist.name}".`, 'info');
            }
        }
    }

    openFullscreenPlaylistModal() {
        if (this.tracks.length === 0) return;
        this.renderFullscreenPlaylistModal();
        this.fullscreenPlaylistModal.classList.remove('hidden');
    }

    closeFullscreenPlaylistModal() {
        this.fullscreenPlaylistModal.classList.add('hidden');
    }

    renderFullscreenPlaylistModal() {
        this.fullscreenPlaylistList.innerHTML = '';
        if (this.playlists.length === 0) {
            this.fullscreenPlaylistList.innerHTML = `<div class="empty-state" style="height: 100px; padding: 10px;"><div class="empty-subtitle">No playlists yet.</div></div>`;
            return;
        }
        ...
        this.playlists.forEach(playlist => {
            ...
            item.addEventListener('click', () => {
                this.addTrackToPlaylist(playlist.id, this.currentIndex);
                this.closeFullscreenPlaylistModal();
            });
            this.fullscreenPlaylistList.appendChild(item);
        });
    }
    ...
    searchTracks(query) {
        ...
        if (this.currentView !== 'library' && query) {
            this.navBtns.forEach(btn => btn.classList.remove('active'));
            document.querySelector('.nav-btn[data-view="library"]').classList.add('active');
            this.currentView = 'library';
        }
        const filteredTracks = this.tracks.filter(track => track.title.toLowerCase().includes(query) || track.artist.toLowerCase().includes(query) || (track.album && track.album.toLowerCase().includes(query)));
        const title = query ? `Search Results for "${query}"` : "Your Library";
        this.contentViewContainer.innerHTML = this.renderLibraryView(filteredTracks, title);
        this.attachEventListenersToView('library');
    }
    handleKeyboard(event) {
        if (event.target.tagName === 'INPUT') return;
        switch (event.code) {
            case 'Space':
                event.preventDefault(); this.togglePlay(); break;
            case 'ArrowLeft':
                if (isNaN(this.audio.duration)) return; event.preventDefault(); this.audio.currentTime = Math.max(0, this.audio.currentTime - (event.shiftKey ? 30 : 10)); break;
            case 'ArrowRight':
                if (isNaN(this.audio.duration)) return; event.preventDefault(); this.audio.currentTime = Math.min(this.audio.duration, this.audio.currentTime + (event.shiftKey ? 30 : 10)); break;
            case 'KeyN':
                if (event.ctrlKey || event.metaKey) return; event.preventDefault(); this.nextTrack(); break;
            case 'KeyP':
                if (event.ctrlKey || event.metaKey) return; event.preventDefault(); this.previousTrack(); break;
            case 'ArrowUp':
                event.preventDefault(); this.setVolumeLevel(this.volume + 0.05); break;
            case 'ArrowDown':
                event.preventDefault(); this.setVolumeLevel(this.volume - 0.05); break;
            case 'KeyS':
                if (event.ctrlKey || event.metaKey) event.preventDefault(); else this.toggleShuffle(); break;
            case 'KeyR':
                if (event.ctrlKey || event.metaKey) event.preventDefault(); else this.toggleRepeat(); break;
            case 'KeyM':
                if (event.ctrlKey || event.metaKey) event.preventDefault(); else this.toggleMute(); break;
            case 'Slash':
                if (event.ctrlKey || event.metaKey) { event.preventDefault(); this.searchInput.focus(); } break;
        }
    }
}
